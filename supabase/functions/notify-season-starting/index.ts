...
serve(async (req) => {
  ...
  try {
    const result = await fanOutSeasonPush({
      supabase,
      seasonId,
      title: 'Season starts in 2 days',
      path: '/app/dashboard',
      logTag: 'notify-season-starting',
      messageFor: (athlete, seasonName) =>
        `${seasonName} begins in 2 days. Your divisions — ${formatDivisionsLine(
    ...
    console.error('[notify-season-starting]', err);
