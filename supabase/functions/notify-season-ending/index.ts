...
serve(async (req) => {
  ...
  try {
    const result = await fanOutSeasonPush({
      supabase,
      seasonId,
      title: 'Season ends in 2 days',
      path: '/app/dashboard',
      logTag: 'notify-season-ending',
      messageFor: (athlete, seasonName) =>
        `${seasonName} ends in 2 days. Current divisions — ${formatDivisionsLine(
    ...
    console.error('[notify-season-ending]', err);
